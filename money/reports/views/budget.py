...
from money.models import AccountingPeriod, Tag, Transaction, TransactionTag
...
class BudgetByPeriodView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        income_groups = (
            Tag.objects.exclude(name="kollektiivi")
            .values("category")
            .distinct()
            .order_by("category")
        )
        ...
        business_transactions = expense_transactions.filter(
            tag__category__in=("business", "design", "rental")
        )
        business_expenses_tags = (
            Tag.objects.filter(category__in=("business", "design", "rental"))
