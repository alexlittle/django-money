...
class Account(models.Model):
    ...
    def on_statement(self):
        trans_cred = Transaction.objects.filter(account=self, on_statement=True).aggregate(
            Sum("credit")
        )
        trans_deb = Transaction.objects.filter(account=self, on_statement=True).aggregate(
            Sum("debit")
        )
        ...
        if tag:
            trans_cred = (
                Transaction.objects.filter(
                    account=account, date__lte=date, transactiontag__tag__name=tag
                )
            ...
            trans_cred = Transaction.objects.filter(account=account, date__lte=date).aggregate(
                credit_sum=Sum("credit")
            )
            trans_deb = Transaction.objects.filter(account=account, date__lte=date).aggregate(
                debit_sum=Sum("debit")
            )
    ...
    def get_monthly_valuation(self):
        v_tmp = Valuation.objects.filter(account=self, value_per_month__gt=0).aggregate(
            date=Max("date")
        )
        if v_tmp["date"] is not None:
            valuation = Valuation.objects.get(
                account=self, value_per_month__gt=0, date=v_tmp["date"]
            )
            return valuation
        else:
            return None
    ...
    def get_valuation_at_date(account, date):
        v_tmp = Valuation.objects.filter(account=account, date__lte=date).aggregate(
            date=Max("date")
        )
        if v_tmp["date"] is not None:
    ...
    def get_paid_in_base_currency_at_date(account, date):
        if account.currency == settings.BASE_CURRENCY:
            credit = Transaction.objects.filter(account=account, date__lte=date).aggregate(
                paid_in=Sum("credit")
            )
            if credit["paid_in"]:
...
class ExchangeRate(models.Model):
    ...
    def most_recent(from_currency, to_currency):
        tmp_date = ExchangeRate.objects.filter(
            from_cur=from_currency, to_cur=to_currency
        ...
        if tmp_date["date"] is not None:
            ...
            return rate.rate
        tmp_date = ExchangeRate.objects.filter(
            from_cur=to_currency, to_cur=from_currency
        ).aggregate(date=Max("date"))
        if tmp_date["date"] is not None:
            rate = ExchangeRate.objects.get(
    ...
    def at_date(date, from_currency, to_currency):
        ...
        tmp_date = ExchangeRate.objects.filter(
            from_cur=from_currency, to_cur=to_currency, date__lte=date
        ).aggregate(date=Max("date"))
        if tmp_date["date"] is not None:
            rate = ExchangeRate.objects.get(
                from_cur=from_currency, to_cur=to_currency, date=tmp_date["date"]
            ...
            return rate.rate
        tmp_date = ExchangeRate.objects.filter(
            from_cur=to_currency, to_cur=from_currency, date__lte=date
        ).aggregate(date=Max("date"))
        if tmp_date["date"] is not None:
            rate = ExchangeRate.objects.get(
                from_cur=to_currency, to_cur=from_currency, date=tmp_date["date"]
...
class Transaction(models.Model):
    ...
    def get_credit_in_base_currency(self):
        if self.account.currency == settings.BASE_CURRENCY:
            return self.credit
        else:
...
class InvoiceTemplate(models.Model):
    ...
    def __str__(self):
        return f"{self.name} - {self.debit_ex_alv} + {self.debit_alv} = {self.debit_total}"
