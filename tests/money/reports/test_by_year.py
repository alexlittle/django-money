...
class ByYearViewTestBase(TestCase):
    @staticmethod
...
class ByYearViewTests(ByYearViewTestBase):
    def test_returns_200_and_uses_by_year_template(self):
        response = self.client.get(reverse("money:by_year"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "money/reports/by_year.html")

    def test_report_has_one_row_per_year_for_last_11_years_newest_first(self):
        # The view builds the list oldest-first then reverses it, so the
        # current year ends up at index 0.
        response = self.client.get(reverse("money:by_year"))
        report = response.context["report"]
        self.assertEqual(len(report), 11)
        self.assertEqual(report[0]["year"], timezone.now().year)

    def test_sums_in_and_out_for_current_year_on_statement_transactions(self):
